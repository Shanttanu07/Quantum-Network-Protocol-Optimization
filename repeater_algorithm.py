...
class RepeaterChainSimulation():
    def __init__(self):
        ...
        self.efficient = True
        self.zero_padding_size =  262144  # Increased padding size to reduce warnings
        self._qutip = False
    ...
    def iterative_convolution_helper(
        ...
        if self.use_fft: # Use geometric sum in Fourier space
            ...
            shape = int(self.zero_padding_size) if self.zero_padding_size is not None else int(2 ** np.ceil(np.log2(2 * trunc - 1)))
            # else:
    ...
    def compute_unit(self,
        ...
        if "cutoff" in parameters.keys():
            ...
            cutoff = parameters.get("mt_cut", np.iinfo(np.int32).max)
        elif cut_type == "fidelity":
            cutoff = parameters.get("w_cut", 1.0e-16)  # shouldn't be zero
            if cutoff == 0.:
                cutoff = 1.0e-16
        elif cut_type == "run_time":
            cutoff = parameters.get("rt_cut", np.iinfo(np.int32).max)
        else:
    ...
    def nested_protocol(self, parameters, all_level=False):
        ...
        if "cutoff_dict" in parameters.keys():
            ...
            mt_cut = cutoff_dict.get("memory_time", np.iinfo(np.int32).max)
            w_cut = cutoff_dict.get("fidelity", 1.e-8)
            rt_cut = cutoff_dict.get("run_time", np.iinfo(np.int32).max)
        else:
            mt_cut = parameters.get("mt_cut", np.iinfo(np.int32).max)
            w_cut = parameters.get("w_cut", 1.e-8)
            rt_cut = parameters.get("rt_cut", np.iinfo(np.int32).max)
