...
def secret_fraction(w):
    fraction = 1 - 2. * entropy((1.-w)/2.)
    return max(fraction, 0.0001)  # Minimum floor value to prevent zero rate
...
def secret_key_rate(pmf, w_func, extrapolation=False, show_warning=False):
    ...
    print("aver_w", aver_w, "aver_t", aver_t)
    ...
    for cut_type in cut_types:
        ...
        if cut_type in ["memory_time", "run_time"]:
            if ref_pmf_matrix is not None:
                ...
                cutoff_pos = cutoff
                cutoff = np.empty(cutoff_pos.shape, dtype=np.int32)
