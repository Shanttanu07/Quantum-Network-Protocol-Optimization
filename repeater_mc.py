        ...
        while not cut_off_pass:
            ...
            if tA < tB:
                if wA < w_cut:
                    ...
                    waiting_time = min(tB, tA + np.int32(np.floor(t_coh * np.log(wA/w_cut))))
            elif tA > tB:
                if wB < w_cut:
                    waiting_time = tB
                else:
                    memory_time = np.int32(np.floor(t_coh * np.log(wB/w_cut)))
...
#                     waiting_time = tB
#                 else:
#                     memory_time = np.int32(np.ceil(t_coh * np.log(wB/w_cut)))
#                     waiting_time = min(tA, tB + memory_time)
#             else:
#                 waiting_time = tA
